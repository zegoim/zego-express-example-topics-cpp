...
#ifndef __ZEGOEXPRESSENGINEINTERFACE_H__
#define __ZEGOEXPRESSENGINEINTERFACE_H__
...
namespace ZEGO {
    ...
    /**
     * 设置延迟模式
     * 在推流前设置才能生效，如果在推流后设置，停推后重新推流可以生效。
     * @param latencyMode 推流的延迟模式
     */
    virtual void setLatancyMode(ZegoLatencyMode latencyMode) = 0;


    /**
     * 设置音频码率
     * 可调用此接口来设置使用指定的音频码率对音频进行编解码。
     * @param bitrate 音频码率值，单位为 bps，默认值为 48 kbps
     */
     ...
     * 拉取不存在的流 ID，执行本接口后 SDK 持续尝试拉取，在该流 ID被成功推送后，音视频流可以真正被拉取到。
     * 开发者可通过再次调用此接口实现更换拉流 Canvas 的操作（流ID必须一样）。
     * @param streamID 流 ID
    ...
    #endif // __ZEGOEXPRESSENGINEINTERFACE_H__
